...
export async function listAuditLogs(options: { limitCount?: number } = {}): Promise<AuditLogRead[]> {
  const snapshot = await getDocs(
    query(auditLogsCollection, orderBy("timestamp", "desc"), limit(options.limitCount ?? 100)),
  );
...
export async function listAuditLogsForUser(userId: string, limitCount = 5): Promise<AuditLogRead[]> {
  const snapshot = await getDocs(
    query(
      auditLogsCollection,
